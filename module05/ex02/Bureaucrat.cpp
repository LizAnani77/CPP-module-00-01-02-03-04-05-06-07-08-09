...
Bureaucrat &Bureaucrat::operator=(const Bureaucrat &rhs)
{
    ...
    if (this != &rhs)
    {
        // Impossible d'affecter l'attribut constant _name
...
// Essaie d'exécuter le formulaire et affiche un message approprié
void Bureaucrat::executeForm(AForm const &form)
{
    try
    {
        // Tente d'exécuter le formulaire par ce bureaucrate
        form.execute(*this);
        // Si l'exécution réussit, affiche un message de succès
        std::cout << _name << " executed " << form.getName() << std::endl;
    }
    catch (std::exception &e)
    {
        // Si l'exécution échoue (exception), affiche un message d'échec
